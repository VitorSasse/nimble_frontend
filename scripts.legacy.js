...
function inicializarPerfil() {
    ...
    const orderDeleteButton = document.getElementById('order-delete');
    ...
    if (orderForm) {
        orderForm.addEventListener('submit', async event => {
            event.preventDefault();
            if (!pedidoEmEdicao || !orderStatusSelect) {
            ...
            const novoStatus = orderStatusSelect.value;
            const pedidoAtualizado = { ...pedidoEmEdicao, status: novoStatus };

            if (orderModalFeedback) {
                orderModalFeedback.textContent = 'Salvando alterações...';
            ...
            try {
                if (supabaseDisponivel && currentUser && pedidoEmEdicao.cliente_id === currentUser.id) {
                    ...
                    pedidosSupabase = pedidosSupabase.map(item => item.id === pedidoAtualizado.id
                        ? { ...item, status: pedidoAtualizado.status }
                        : item
                    );
                }
                ...
                renderDashboardVendedor();
                if (orderModalFeedback) {
                    orderModalFeedback.textContent = 'Pedido atualizado com sucesso!';
    ...
    if (orderDeleteButton) {
        orderDeleteButton.addEventListener('click', () => {
            if (pedidoEmEdicao) {
                removerPedidoCliente(pedidoEmEdicao.id);
            }
        });
    }
    ...
    if (productForm) {
        productForm.addEventListener('submit', async event => {
            ...
            if (!novoProduto.nome || !novoProduto.descricao || !novoProduto.categoria || Number.isNaN(precoNumerico)) {
    ...
    function popularStatusPedidos() {
        if (!orderStatusSelect) return;
        if (orderStatusSelect.options.length) return;
        orderStatusOptions.forEach(opcao => {
    ...
    function abrirModalPedido(pedidoId) {
        ...
        if (orderStatusSelect) {
            orderStatusSelect.value = pedido.status || 'processando';
        }
    ...
    async function carregarProdutosSupabase() {
        ...
        try {
            const { data, error } = await supabase
                .from('produtos')
                .select('id, nome, descricao, categoria, preco, imagem, owner_id, created_at')
...
async function inicializarCarrinho() {
    ...
    if (confirmarButton) {
        ...
        confirmarButton.addEventListener('click', async () => {
            ...
            const pedido = {
                ...
                status: pagamento.detalhes.status,
    ...
    function atualizarResumoEndereco(endereco) {
        ...
        if (endereco && endereco.logradouro) {
            addressInfo.textContent = formatarEnderecoCurto(endereco);
            return;
        }
    ...
    function coletarDadosPagamento() {
        ...
        if (metodo === 'pix') {
            ...
            const pixName = (pixNameInput?.value || '').trim();
            if (pixKey.length < 6) {
                return { valido: false, mensagem: 'Informe a chave PIX utilizada no app do banco.' };
            }
            if (!pixName) {
                return { valido: false, mensagem: 'Informe o nome do titular da conta.' };
            }
            return {
                valido: true,
                detalhes: {
                    metodo: 'pix',
                    status: 'pago',
                    descricao: `PIX confirmado por ${pixName}`,
                    detalhes: {
                        chave: pixKey,
                        titular: pixName
                    }
